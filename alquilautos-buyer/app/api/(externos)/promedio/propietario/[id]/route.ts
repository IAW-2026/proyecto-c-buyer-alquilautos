...
//Obtiene la calificación promedio y cantidad de reseñas de un propietario consultando a la Feedback App (VERIFICADO)

export async function GET(_req: Request, { params }: Props) {
  const { id } = await params;
  // TODO: const res = await fetch(`${process.env.FEEDBACK_APP_URL}/api/promedio/propietario/${id}`);
  // return NextResponse.json(await res.json());
  const cal = calificacionesPropietarios.find((c) => c.id_propietario === id);
